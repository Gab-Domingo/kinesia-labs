...
const isLightObject = (object: THREE.Object3D): object is THREE.Light => {
  return 'isLight' in object && (object as any).isLight === true;
};
...
const ModelInnerBase: FC<ModelInnerBaseProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    g.traverse((o: THREE.Object3D) => {
      if (isMeshObject(o)) {
        ...
        if (fadeIn) {
          const materials = Array.isArray((o as any).material) ? (o as any).material : [(o as any).material];
  ...
  useEffect(() => {
    ...
    const down = (e: PointerEvent) => {
      ...
      if (pts.size === 1) {
        ...
        startZ = (camera as any).position.z;
        e.preventDefault();
      }
      invalidate();
    };
    ...
    const move = (e: PointerEvent) => {
      ...
      if (mode === 'rotate') {
        ...
        (camera as any).position.z = THREE.MathUtils.clamp(startZ * ratio, minZoom, maxZoom);
        invalidate();
      }
    };
    ...
    el.addEventListener('pointerdown', down, { passive: true } as any);
    window.addEventListener('pointermove', move as any, { passive: false } as any);
    window.addEventListener('pointerup', up as any, { passive: true } as any);
    window.addEventListener('pointercancel', up as any, { passive: true } as any);
    return () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('pointermove', mm as any);
    return () => window.removeEventListener('pointermove', mm as any);
...
const ModelViewer: FC<ViewerProps> = ({
  ...
  const capture = () => {
    ...
    s.traverse((o: THREE.Object3D) => {
      if (isLightObject(o)) {
        tmp.push({ l: o as THREE.Light, cast: (o as any).castShadow });
        (o as any).castShadow = false;
      }
    });
    if (contactRef.current) (contactRef.current as any).visible = false;
    g.render(s, c as any);
    ...
    if (contactRef.current) (contactRef.current as any).visible = true;
    invalidate();
  };
